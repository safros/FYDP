...
#declare variables
demand_Retailer = [0,100,150,100] #the first entry is the depot
...
m.addConstrs(sum(x_ijk[i,j,k] for j in rangePoints for k in rangeNumVehicle )==1 for i in range(1,4))
#(3) each customer should be visited once by any vehicle if the customer is assigned to that the route of the vehicle
m.addConstrs(sum(x_ijk[i,j,k] for i in rangePoints for k in rangeNumVehicle )==1 for j in range(1,4) )
...
m.addConstrs(sum(demand_Retailer[j]*x_ijk[i,j,k] for i in rangePoints for j in rangePoints )<= capacity_tuck_k[k] for k in rangeNumVehicle)
#(7)forbids the allocation of the same node by different vehicles
m.addConstrs(sum(x_ijk[i,j,k]*demand_Retailer[j] for i in rangePoints for j in rangePoints ) - sum(x_ijk[i,j,k]*demand_Retailer[i] for i in rangePoints for j in rangePoints if i!=j)==0 for k in rangeNumVehicle)

#m.addConstrs(sum(x_ijk[i,j,k] for i in rangePoints if i!=j)==sum(x_ijk[j,i,k] for i in rangePoints ) for j in rangePoints for k in rangeNumVehicle)
#m.addConstrs(sum(x_ijk[i,j,k] for i in rangePoints for k in rangeNumVehicle )==1 for j in range(1,4))
#m.addConstrs(sum(x_ijk [0,j,k] for j in range(1,4))==1 for k in rangeNumVehicle)
#m.addConstrs(sum(demand_Retailer[j-1]*x_ijk[i,j,k] for i in rangePoints for j in range(1,4) )<= capacity_tuck_k[k] for k in rangeNumVehicle)
